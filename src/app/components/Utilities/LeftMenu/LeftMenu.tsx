...
export default function LeftMenu(openMenu:boolean){
    ...
    return (
        // <motion.div initial={{x:'-140vw'}} animate={{x:'-50vw'}} transition={{ delay: 1 }}>
        <div className={`${styles.menu}`} style={{
            display: openMenu? 'inherit': 'none'
        }}>
            ...
            <div className={`${styles.menu__accountContainer}`}>
                <div className={`${styles.menu__account}`}>
                    ...
                    <div className={`${styles.account__logout}`}>{!userDetails["data"]["user"]? <Link href={"/components/LogIn"}>Log in</Link>: <Link href={'/components/HomeScreen'}><div onClick={()=>services.logOut()}>Log Out</div></Link>}</div>
                </div>
            </div>
        </div>
    //</motion.div>
